...
get_stat(Champ) ->
    % F = fun({Team, _, Players}) ->
    %        {TeamAge, TeamRaiting} = lists:foldl(
    %             fun({_, _, Age, Raiting, _}, {TotalAge, TotalRaiting}) ->
    %                 {TotalAge + Age, TotalRaiting + Raiting}
    %              end, {0, 0}, Players),
    %     {length(Players), TeamAge, TeamRaiting},

    Stats = lists:map(fun each_team_stats/1, Champ),
    {TotalPlayers, TotalAge, TotalRaiting} =
        lists:foldl( 
            fun({TeamPlayers, TeamAge, TeamRaiting},
                {AllPlayers, AllAges, AllRaitings}) ->
                {TeamPlayers + AllPlayers, TeamAge + AllAges, TeamRaiting + AllRaitings}
            end,
            {0, 0, 0},
            Stats),
    {length(Champ), TotalPlayers, TotalAge / TotalPlayers, TotalRaiting / TotalPlayers}.

each_team_stats({_, _, Players}) ->
        % F = fun({Team, _, Players}) ->
            {TeamAge, TeamRaiting} = lists:foldl(
                fun({_, _, Age, Raiting, _}, {TotalAge, TotalRaiting}) ->
                    {TotalAge + Age, TotalRaiting + Raiting}
...
filter_sick_players(Champ) ->
    Health = lists:map(fun filter_healthy/1, Champ),
    lists:filter(
        fun({_, _, Players}) ->
            length(Players) >= 5 end, Health).

filter_healthy({Team, Name, Players}) ->
    HealthyTeam =
        lists:filter(
            fun({_, _, _, _, Health}) -> Health >= 50 end, Players),
        {Team, Name, HealthyTeam}.
...
make_pairs(Team1, Team2) ->
    {_, _, Players1} = Team1, 
    {_, _, Players2} = Team2, 
    [{Name1, Name2}|| {_, Name1, _, Raiting1, _} <- Players1,
                          {_, Name2, _, Raiting2, _} <- Players2,
                           Raiting1 + Raiting2 > 600].
